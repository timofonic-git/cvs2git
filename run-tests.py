...
def mixed_time_tag():
  ...
  expected = (
    ('/%(tags)s/T_MIXED (from /%(branches)s/B_MIXED:32)', 'A'),
    )
  if log.revision == 16:
...
def default_branches():
  ...
  conv.logs[18].check(sym_log_msg('vtag-4',1), (
    ...
    ('/%(tags)s/vtag-4/proj/d.txt '
     '(from /%(trunk)s/proj/d.txt:17)', 'A'),
    ))

  conv.logs[6].check(sym_log_msg('vtag-1',1), (
    ('/%(tags)s/vtag-1 (from /%(trunk)s:5)', 'A'),
    ('/%(tags)s/vtag-1/proj/e.txt', 'D'),
    ))

  conv.logs[9].check(sym_log_msg('vtag-2',1), (
    ('/%(tags)s/vtag-2 (from /%(trunk)s:8)', 'A'),
    ('/%(tags)s/vtag-2/proj/e.txt', 'D'),
    ))

  conv.logs[12].check(sym_log_msg('vtag-3',1), (
    ('/%(tags)s/vtag-3 (from /%(trunk)s:11)', 'A'),
    ))
